package us.xdev.mediaplayer.views
{
	...
	public class ThumbGrid extends AbstractView
	{
		...
		public function ThumbGrid(ref:Object, model:*, controller:*, obj:Object=null)
		{
		...
		private function handleIconsMouse(event:MouseEvent):void
		{
			var mc:MovieClip = MovieClip(event.currentTarget);
			if(event.type == MouseEvent.CLICK){
				advancePage(mc.dir);
			...
			if(event.type == MouseEvent.MOUSE_OVER){
				TweenLite.to(mc, 0.2, { scaleX:1.2, scaleY:1.2 });
			}
			if(e.type == MouseEvent.MOUSE_OUT){
				TweenLite.to(mc, 0.5, { scaleX:1.0, scaleY:1.0 });
			}
		}
		...
		private function revealThumb(event:CustomEvent):void
		{
			var mc:MovieClip = event.props.mc;
