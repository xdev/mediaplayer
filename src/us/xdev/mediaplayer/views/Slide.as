package us.xdev.mediaplayer.views
{
	...
	public class Slide extends AbstractView
	{
		...
		protected function handleStillLoad(event:Event):void
		{
		...
		private function reveal(event:Event=null):void
		{
		...
		private function handlePreload(event:ProgressEvent):void
		{
			var p:int = Math.ceil(100*(event.bytesLoaded / event.bytesTotal));
		...
		{
			return Utils.getScale(
				objWidth,
				objHeight,
				ref.stage.stageWidth - (model.getConfig().marginX*2),
				ref.stage.stageHeight - (model.getConfig().marginY*2),
				'scale',
				max
			).x;
		}
		...
		public function scale():void
		{
			...
			if(still){
				...
				//TODO: consolidate scaling of still image
